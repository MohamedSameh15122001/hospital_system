...
class MangerSearchManger extends StatelessWidget {
  MangerSearchManger({super.key});
  final FocusNode _focusNode = FocusNode();
  @override
  Widget build(BuildContext context) {
    FocusScope.of(context).requestFocus(_focusNode);
    return BlocConsumer<ManagerCubit, ManagerState>(
      bloc: ManagerCubit.get(context)..searchMangers(token: token!, input: ''),
                  ...
                  children: [
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Search Manger With Name Or ID',
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(20.0),
                        ),
                      ),
                      onChanged: (value) async {
                        await cubit.searchMangers(token: token!, input: value);
                      },
                    ),
                    const SizedBox(height: 20),
                    state is LoadingSearchMangers
                        ? Center(
                            child: loading,
                          )
                        : state is ErrorSearchMangers
                            ? Center(
                                child: Text(cubit.errorModel!.message!),
                              )
                            : cubit.searchGetAllMangersModel!.result!.isEmpty
                                ? const Center(
                                ...
                                : Expanded(
                                    child: ListView.builder(
                                      ...
                                      itemCount: cubit.searchGetAllMangersModel!
                                          .result!.length,
                                      itemBuilder: (context, index) {
                                        var model = cubit
                                            .searchGetAllMangersModel!
                                            .result![index];
