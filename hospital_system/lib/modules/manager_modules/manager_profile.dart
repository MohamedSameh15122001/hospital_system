...
import 'package:hospital_system/modules/manager_modules/manager_change_password.dart';
...
class MangerProfile extends StatelessWidget {
  const MangerProfile({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<ManagerCubit, ManagerState>(
      bloc: ManagerCubit.get(context)..getMangerProfile(token: token!),
      listener: (context, state) {},
      builder: (context, state) {
        internetConection('manger', context);
        ManagerCubit cubit = ManagerCubit.get(context);
        var model = cubit.getMangerProfileModel?.data;
        return Scaffold(
          appBar: AppBar(
          ...
          body: state is LoadingGetMangerProfile
              ? Center(child: loading)
              : state is ErrorGetMangerProfile
                  ? Center(
                      child: Text(cubit.errorModel!.message!),
                    )
                  : cubit.getMangerProfileModel == null
                      ? Center(
                          child: loading,
                        )
